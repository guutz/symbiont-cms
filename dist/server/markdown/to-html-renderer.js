...
const slugify = (text, options) => {
    const fn = typeof slugifyFn === 'function' ? slugifyFn : slugifyFn.default;
 ...
 */
export async function renderMarkdownToHtml(content, config) {
    const toc = [];
...
function addToTOC(toc, level, heading, slugUrl) {
 ...
 */
export function renderSummaryToHtml(content) {
    const md = new MarkdownIt({
        html: true,
        linkify: false,
...
//# sourceMappingURL=to-html-renderer.js.map
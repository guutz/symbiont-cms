...
export class NotionToDatabaseSync {
    ...
    pageCrud;
    config;
    logger = createLogger({ operation: 'notion_to_database_sync' });
    constructor(notionClient, pageTransformer, pageCrud, config) {
    ...
    async syncDataSource(options = {}) {
        ...
        try {
            ...
            do {
                const result = await this.notionClient.queryDataSource(this.config.dataSourceId, filter, cursor);

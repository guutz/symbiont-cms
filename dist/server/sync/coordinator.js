 ...
 */
export function createNotionToDatabaseSyncCoordinator(client, config, adminSupabase, extraHooks = []) {
    ...
    notionClient.setBlockTransformer('image', async (block) => {
        const { image } = block;
        if (!image?.type)
    ...
    const supabase = adminSupabase ?? createClient(client.config.supabase.url, requireEnvVar("SUPABASE_SERVICE_ROLE_KEY"), {
        auth: {
    ...
    const transformer = new NotionPageToDatabasePageTransformer(config, notionClient, pageCrud, supabase, extraHooks);

 ...
 */
function extractReferencedPaths(text, out) {
 ...
 */
async function listAllMediaPaths(supabase, prefix = '', logger) {
    ...
    while (true) {
        ...
        if (error)
            throw new Error(`Failed to list media bucket (prefix "${prefix}"): ${error.message}`);
 ...
 */
export async function cleanupUnusedMedia(supabase, options = {}) {
    ...
    while (true) {
        ...
        for (const page of pages) {
            if (page.content)
                extractReferencedPaths(page.content, referencedPaths);
            if (page.cover)
                extractReferencedPaths(page.cover, referencedPaths);
        }

...
export class DatabasePageCRUD {
    logger = createLogger({ operation: 'database_page_crud' });
    supabase;
    static PAGE_ID_CHUNK_SIZE = 500;
    /**
     ...
     */
    async getByNotionPageId(pageId) {
        this.logger.debug({
     ...
     */
    async getSyncRefsByPageIds(pageIds) {
        const refs = new Map();
        if (pageIds.length === 0) {
            return refs;
        }
     ...
     */
    async getBySlug(slug, datasourceId) {
        this.logger.debug({
            event: 'get_by_slug',
            slug,
            datasourceId
     ...
     */
    async getAllForSource(datasourceId) {
        this.logger.debug({
     ...
     */
    async deleteForSource(datasourceId) {
        this.logger.info({

...
function normalizeRichTextArray(rts) {
 ...
 */
export function normalizeBlockForDiff(block) {
 ...
 */
export function blocksAreEquivalent(existingBlocks, newBlocks) {
    ...
    if (existingBlocks.some((b) => b?.has_children === true))
        return false;
    ...
    if (normalizedExisting.some((b) => b?._file || b?._unknown))
        return false;
    if (normalizedNew.some((b) => b?._file || b?._unknown))
        return false;
 ...
 */
export function fingerprintBlock(block) {
    return {
        id: block?.id,
 ...
 */
export function diffBlocks(existing, desired, forceFullReplaceThreshold = 0.6) {
    const ops = [];
    ...
    while (i < ef.length && j < df.length) {
        ...
        if (e.type === d.type) {
            ...
            const desiredContent = d.raw[d.type];
            const desiredHasChildren = Array.isArray(desiredContent?.children) && desiredContent.children.length > 0;
            const identical = !e.hasChildren &&
                !desiredHasChildren &&
                e.normalized !== null &&
                d.normalized !== null &&
                !e.normalized._file &&
        ...
        for (let k = 1; k <= LOOKAHEAD; k++) {
            ...
            if (matchInExisting !== -1 && matchInDesired !== -1)
                break;
        }

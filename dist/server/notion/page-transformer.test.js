...
describe('NotionPageToDatabasePageTransformer', () => {
    it('keeps publish_at explicitly null when publish:date resolves to no value', async () => {
        ...
        const supabase = {};
        const transformer = new NotionPageToDatabasePageTransformer(config, notionClient, pageCrud, supabase);
        const page = {
            id: 'page-123',
        ...
        expect(pageCrud.upsert).toHaveBeenCalledWith(expect.objectContaining({
            page_id: 'page-123',
            publish_at: null,
        }));
    });

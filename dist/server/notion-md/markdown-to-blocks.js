...
import { richText, ensureLength, } from './rich-text.js';
...
function extractDoubleDollarMath(markdown) {
    const expressions = [];
    const processed = markdown.replace(/\$\$([\s\S]*?)\$\$/g, (_, expr) => {
...
function isSoleMathSentinel(text) {
    return /^SYMBIONT_MATH_\d+ENDMATH$/.test(text);
}
function getSentinelIndex(text) {
 ...
 */
function splitBySentinel(text, expressions) {
    const parts = [];
    let lastIndex = 0;
    let match;
...
function linkToPageBlock(pageId) {
    return {
        object: 'block',
        type: 'link_to_page',
...
function mentionPageRichText(pageId) {
    return {
        type: 'mention',
        mention: { type: 'page', page: { id: pageId } },
    };
}
function mentionLinkPreviewRichText(url) {
    return {
        type: 'mention',
...
function tableBlock(rows, tableWidth) {
    return {
        object: 'block',
        type: 'table',
        table: {
...
function getInlinePlainText(node) {
    switch (node?.type) {
        case 'text':
        case 'inlineCode':
        case 'html':
        ...
        case 'strong':
        case 'link':
            return (node.children ?? []).map(getInlinePlainText).join('');
        default:
            return '';
    }
}
function parseInlineText(text, options, expressions) {
    const parts = splitBySentinel(text, expressions);
    return parts.flatMap(part => {
        if (part.type === 'math') {
            return [richText(part.value, { type: 'equation', annotations: options.annotations })];
        }
        if (!part.value)
            return [];
        return ensureLength(part.value, { annotations: options.annotations, url: options.url });
    });
}
function parseInline(node, options, expressions) {
    const copy = {
    ...
    switch (node.type) {
        case 'text':
            return parseInlineText(node.value, copy, expressions);
        case 'delete':
            return node.children.flatMap(child => parseInline(child, { ...copy, annotations: { ...copy.annotations, strikethrough: true } }, expressions));
        case 'emphasis':
            return node.children.flatMap(child => parseInline(child, { ...copy, annotations: { ...copy.annotations, italic: true } }, expressions));
        case 'strong':
            return node.children.flatMap(child => parseInline(child, { ...copy, annotations: { ...copy.annotations, bold: true } }, expressions));
        case 'link': {
            const notionPageId = extractNotionPageId(node.url ?? '');
            if (notionPageId) {
            ...
            return node.children.flatMap(child => parseInline(child, { ...copy, url: node.url }, expressions));
        }
        case 'inlineCode':
            return [richText(node.value, { ...copy, annotations: { ...copy.annotations, code: true } })];
        case 'html':
            // Inline HTML — pass through as plain text
            return ensureLength(node.value, copy);
        default:
            return [];
    }
}
...
// ── Image helper ─────────────────────────────────────────────────────────────
function parseImage(node, strictImageUrls) {
    const url = node.url ?? '';
    if (!url)
        return paragraph([richText(node.alt ?? '')]);
...
// ── Paragraph parser ─────────────────────────────────────────────────────────
function parseParagraph(node, options, expressions) {
    ...
    if (children.length === 1 && children[0].type === 'link') {
        const pageId = extractNotionPageId(children[0].url ?? '');
        if (pageId)
            return [linkToPageBlock(pageId)];
    }
...
// ── Blockquote parser ────────────────────────────────────────────────────────
function parseBlockquote(node, options, expressions) {
    const firstChild = node.children?.[0];
    const firstText = firstChild?.type === 'paragraph' ? firstChild.children?.[0] : null;
    if (firstText?.type === 'text') {
        const parseSubsequent = () => node.children.length > 1
            ? node.children.slice(1).flatMap((child) => parseNode(child, options, expressions))
            : [];
        ...
        if (gfmMatch && SUPPORTED_GFM_ALERT_TYPES.includes(gfmMatch[1])) {
            ...
            return callout([richText(displayType)], alertConfig.emoji, alertConfig.color, alertChildren);
        }
    }
    const blockChildren = node.children.flatMap((child) => parseNode(child, options, expressions));
    return blockquote([], blockChildren);
}
// ── Other block parsers ──────────────────────────────────────────────────────
function parseHeading(node, expressions) {
    const text = (node.children ?? []).flatMap((child) => parseInline(child, {}, expressions));
    switch (node.depth) {
...
function parseList(node, options, expressions) {
    return (node.children ?? []).flatMap((item) => {
        const children = [...(item.children ?? [])];
        const paragraphNode = children.shift();
        if (!paragraphNode || paragraphNode.type !== 'paragraph')
            return [];
        const text = (paragraphNode.children ?? []).flatMap((child) => parseInline(child, {}, expressions));
        const parsedChildren = children.flatMap((child) => parseNode(child, options, expressions));
...
function parseTable(node, expressions) {
    const rows = (node.children ?? []).map((rowNode) => {
        const cells = (rowNode.children ?? []).map((cellNode) => (cellNode.children ?? []).flatMap((child) => parseInline(child, {}, expressions)));
        return tableRow(cells);
    });
    const tableWidth = node.children?.[0]?.children?.length ?? 0;
    return [tableBlock(rows, tableWidth)];
}
function parseNode(node, options, expressions) {
    switch (node.type) {
        case 'heading':
 ...
 */
export function convertMarkdownToNotionBlocks(markdown, options) {
    if (!markdown?.trim())
        return [];
    ...
    const root = unified().use(remarkParse).use(remarkGfm).parse(processed);
    // Step 3: Convert AST to Notion blocks
    const parsed = (root.children ?? []).flatMap((node) => parseNode(node, options ?? {}, expressions));

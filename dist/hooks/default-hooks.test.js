...
import { defaultPublishCheckHook, defaultPublishDateHook, defaultSlugExtractHook, defaultSlugGenerateHook, defaultPageShouldSyncHook, defaultTitleExtractHook, defaultTagsExtractHook, defaultAuthorsExtractHook, defaultSummaryExtractHook, defaultCustomMetadataHook, defaultContentPostprocessHook, } from './default-hooks.js';
describe('Default Hooks (Extractor Pattern)', () => {
    ...
    describe('defaultPublishCheckHook', () => {
        it('should return true by default', async () => {
            // Without a notionClient, the default hook passes the vote to publish (returns true) to avoid blocking content. This allows users to implement their own publish logic based on other criteria without being overridden by a default that checks for a specific Notion property.
    ...
    describe('defaultContentPostprocessHook (resolveNotionPageLinks)', () => {
        ...
        it('returns content unchanged when there are no notion:// sentinels', async () => {
            ...
            const ctx = createMockContext({
                input: content,
                services: { supabase: mockSupabase([]) },
            });
            const result = await defaultContentPostprocessHook.fn(ctx);
            expect(result).toBe(content);
        });
        it('resolves a found page sentinel to /{slug}', async () => {
            ...
            const ctx = createMockContext({
                input: content,
                services: {
                    supabase: mockSupabase([{ page_id: UUID, slug: 'my-article' }]),
                },
            });
            const result = await defaultContentPostprocessHook.fn(ctx);
        ...
        it('strips a sentinel whose DB row has null slug to plain text', async () => {
            ...
            const ctx = createMockContext({
                input: content,
                services: {
                    supabase: mockSupabase([{ page_id: UUID, slug: null }]),
                },
            });
            const result = await defaultContentPostprocessHook.fn(ctx);
        ...
        it('resolves duplicate sentinels (same ID used twice)', async () => {
            ...
            const ctx = createMockContext({
                input: content,
                services: {
                    supabase: mockSupabase([{ page_id: UUID, slug: 'page-a' }]),
                },
            });
            const result = await defaultContentPostprocessHook.fn(ctx);

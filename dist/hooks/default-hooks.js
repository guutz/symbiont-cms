 ...
 */
// ── Page Lifecycle ─────────────────────────────────────────────────
export const defaultPageBeforeHook = {
...
export const defaultPageShouldSyncHook = {
...
export const defaultPageAfterHook = {
...
export const defaultPublishCheckHook = {
...
export const defaultPublishDateHook = {
...
// ── Slug Pipeline ──────────────────────────────────────────────────
export const defaultSlugExtractHook = {
...
export const defaultSlugGenerateHook = {
    ...
    fn: async (ctx) => {
        ...
        const titleProp = Object.values(ctx.page.properties).find((p) => p.type === 'title');
...
export const defaultSlugConflictHook = {
    ...
    fn: async (ctx) => {
        ...
        if (!ctx.syncStore[cacheKey]) {
            ...
            const map = new Map((allPages ?? [])
                .filter((p) => p.slug)
                .map((p) => [p.slug, p.page_id]));
        ...
        const slugMap = ctx.syncStore[cacheKey];
...
export const defaultSlugSyncHook = {
...
// ── Metadata Extraction ────────────────────────────────────────────
export const defaultTitleExtractHook = {
...
export const defaultTagsExtractHook = {
...
export const defaultAuthorsExtractHook = {
    ...
    fn: async (ctx) => {
        ...
        if (authorsProp && 'people' in authorsProp) {
            return authorsProp.people?.map((person) => person.name || person.id) || [];
        }
        // Handle multi_select property
        if (authorsProp && 'multi_select' in authorsProp) {
            return authorsProp.multi_select?.map((tag) => tag.name) || [];
        }
        return [];
    }
};
export const defaultSummaryExtractHook = {
...
export const defaultCustomMetadataHook = {
...
// ── Content Pipeline ───────────────────────────────────────────────
export const defaultContentPreprocessHook = {
...
export const defaultContentTextHook = {
...
export const defaultContentMediaHook = {
...
export const defaultContentPostprocessHook = {
    ...
    fn: async (ctx) => {
        ...
        const sentinelMatches = Array.from(content.matchAll(/\[([^\]]+)\]\(notion:\/\/page\/([0-9a-f]{32})\)/gi));
        ...
        const uuids = uniqueCleanIds.map(c => `${c.slice(0, 8)}-${c.slice(8, 12)}-${c.slice(12, 16)}-${c.slice(16, 20)}-${c.slice(20)}`);
        ...
        return content.replace(/\[([^\]]+)\]\(notion:\/\/page\/([0-9a-f]{32})\)/gi, (_, label, cleanId) => {
            const slug = slugMap.get(cleanId.toLowerCase()) ?? null;
            return slug ? `[${label}](/${slug})` : label;
        });
    }
};
export const defaultContentSyncHook = {
...
// ── Cover Pipeline ─────────────────────────────────────────────────
export const defaultCoverExtractHook = {
...
export const defaultCoverProcessHook = {
    name: 'symbiont:cover:process',
    event: 'cover:process',
    fn: async (ctx) => {
        const coverUrl = ctx.input;
        const supabase = ctx.services.supabase;
...
export const defaultCoverSyncHook = {
    ...
    fn: async (ctx) => {
        ...
        const existingCoverUrl = existingCoverProp && 'files' in existingCoverProp
            ? (existingCoverProp.files?.[0]?.file?.url
                ?? existingCoverProp.files?.[0]?.external?.url
                ?? null)
            : null;

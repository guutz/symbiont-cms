...
export class HookRegistry {
    ...
    config;
    services;
    constructor(logger, config, services) {
        this.logger = logger;
        this.config = config;
     ...
     */
    async execute(event, output, page, input) {
        ...
        switch (strategy) {
            ...
            case CompositionStrategy.AndAll:
                result = await this.executeAndAll(hooks, output, page, input, state, abort, event === 'publish:check');
                break;
     ...
     */
    async executeFirstWins(hooks, output, page, input, state, abort) {
        for (const hook of hooks) {
            if (state.aborted) {
     ...
     */
    async executeCollect(hooks, output, page, input, state, abort) {
        let result = null;
        let resultType = null;
        for (const hook of hooks) {
            if (state.aborted) {
     ...
     */
    async executeOrAll(hooks, output, page, input, state, abort) {
        let hasTrue = false;
        for (const hook of hooks) {
            if (state.aborted) {
                this.throwAbort(hook, state);
     ...
     */
    async executeAndAll(hooks, output, page, input, state, abort, requireExplicitTrueVote = false) {
        let hasFalse = false;
        let hasTrue = false;
        for (const hook of hooks) {
            if (state.aborted) {
     ...
     */
    async executeRunAll(hooks, output, page, input, state, abort) {
        for (const hook of hooks) {
            if (state.aborted) {
                this.throwAbort(hook, state);
            }
     ...
     */
    async executePipeline(hooks, output, page, initialValue, state, abort) {
     ...
     */
    buildContext(output, page, input, state, abort) {
        return {
            page,
     ...
     */
    handleHookError(hook, error, state) {
        this.logger.error({
    ...
    mapPriority(priority) {
        if (priority === 'before' || priority === 'override')
            return 40;
        if (priority === 'after' || priority === 'fallback')
            return 60;
        return 50; // default
    }
    /**
